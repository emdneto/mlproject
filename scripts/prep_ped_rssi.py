...
class Prepare:
    ...
    def preparing(self):
        ...
        print('--------------------------------------------------------------------')
        csvFiles = listdir(self.datasetPedestrianPath)
        for csvFile in csvFiles:
            filepath = path.join(self.datasetPedestrianPath, csvFile)
...
'ServingCell_Distance', 'WeekDay', 'DayPeriod', 'Month', 'RSSI']]
            dataset.to_csv(path.join(self.modifiedPath, 'pedestrian_rssi.csv'), mode='a', header=False)
